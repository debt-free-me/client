...
import {
  from, of, Observable, BehaviorSubject, combineLatest, throwError
} from 'rxjs';
import { tap, catchError, concatMap, shareReplay } from 'rxjs/operators';
import createAuth0Client from '@auth0/auth0-spa-js';
import Auth0Client from '@auth0/auth0-spa-js/dist/typings/Auth0Client';
import { AuthDBConnection, AuthUser } from '../typings/auth.typing';
...
export class AuthService {
  ...
  async init(domainUrl: string, clientId: string) {
    ...
    this.client = await this.auth0Client$.toPromise();
  }
  ...
  async signup(email: string, password: string): Promise<AuthUser> {
    try {
      const user: AuthUser = <AuthUser>await this.httpClient.post(
        `${this.domainUrl}/dbconnections/signup`,
        {
          client_id: this.clientId,
          email,
          password,
          connection: AuthDBConnection.UsernamePassword,
        }).toPromise();
      return user;
    } catch (error) {
      throw error;
